...
pub fn write_prompt_file(prompt: &str) -> Result<PathBuf> {
    ...
    let path = home.join("last-agent-prompt.md");
    fs::write(&path, prompt).with_context(|| format!("write {}", path.display()))?;
    // Keep legacy filename for older docs / muscle memory.
...
pub fn spawn_in_new_terminal(host: TerminalHost, shell_cmd: &str) -> Result<String> {
    ...
    match host {
        TerminalHost::MacTerminal => spawn_macos_terminal(shell_cmd),
        TerminalHost::ITerm => spawn_iterm(shell_cmd),
...
pub fn write_run_script(shell_cmd: &str) -> Result<PathBuf> {
    ...
    let path = home.join("last-agent-run.command");
    let body = format!(
        "#!/bin/zsh\n\
...
fn spawn_macos_terminal(shell_cmd: &str) -> Result<String> {
    ...
    let script_path = write_run_script(shell_cmd)?;
...
fn spawn_iterm(shell_cmd: &str) -> Result<String> {
...
pub fn launch_agent(
    ...
    let prompt_path = write_prompt_file(&body)?;
    ...
    let host_label = spawn_in_new_terminal(terminal, &shell_cmd)?;
    Ok(LaunchReport {
        agent,
        grok_mode,
        prompt_path,
